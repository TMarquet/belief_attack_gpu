...
if __name__ == "__main__":
    ...
    for i in range(17,33):
        variables_to_test.append('xt0'+ ('0'+str(i) if i < 10 else '' + str(i)))
                ...
                for var in variables_to_test :
                    for (m) in sorted(listdir(MODEL_FOLDER)):
                        ...
                        if int(var_number) <= 16:
                            ...
                            if string_starts_with(m, 'all_{}_cnn'.format(var_name) if var_name == 's' or var_name == 'xt' else 'all_{}_2'.format(var_name)):
                                print 'Testing : ', m 
                                print(var)
                                if model is None:
                                    model = load_sca_model(MODEL_FOLDER + m)
