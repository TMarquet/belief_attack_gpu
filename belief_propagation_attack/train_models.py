...
def train_model(X_profiling, Y_profiling, model, save_file_name, epochs=150, batch_size=100, validation_data=None, progress_bar=1, hamming_distance_encoding=False, one_hot=True, multilabel=False, hammingweight=False):
    ...
    print len(Reshaped_X_profiling)
    print len(reshaped_y)
    print len(Reshaped_validation_data)
    print len(reshaped_val)
    data = tf.data.Dataset.from_tensor_slices((Reshaped_X_profiling,reshaped_y)).shuffle().batch(batch_size=batch_size,drop_remainder=True)
    

    
    validation_data = tf.data.Dataset.from_tensor_slices((Reshaped_validation_data,reshaped_val)).shuffle().batch(batch_size=batch_size,drop_remainder=True)
