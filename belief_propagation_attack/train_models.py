...
def cnn_best(input_length=2000, learning_rate=0.00001, classes=256, dense_units=4096):
    ...
    # Convolution Blocks
    # Block 1
    model.add(Conv1D(64, 5, padding='same', name='block1_conv1',input_shape = input_shape))
    ...
    # Block 2
    model.add(Conv1D(128, 5, padding='same', name='block2_conv1'))    
    model.add(Lambda(lambda x: K.l2_normalize(x,axis=1)))
    ...
    # Block 3
    model.add(Conv1D(256, 5, padding='same', name='block3_conv1'))
    model.add(Lambda(lambda x: K.l2_normalize(x,axis=1)))
    ...
    # Block 4
    model.add(Conv1D(512, 5, padding='same', name='block4_conv1'))
    model.add(Lambda(lambda x: K.l2_normalize(x,axis=1)))
    ...
    # Block 5
    model.add(Conv1D(512, 5, padding='same', name='block5_conv1'))
    model.add(Lambda(lambda x: K.l2_normalize(x,axis=1)))
