...
def train_model(X_profiling, Y_profiling, model, save_file_name, epochs=150, batch_size=100, validation_data=None, progress_bar=1, hamming_distance_encoding=False, one_hot=True, multilabel=False, hammingweight=False):
    ...
    history = model.fit(x=Reshaped_X_profiling, y=reshaped_y,steps_per_epoch=3800,validation_steps=200, batch_size=batch_size, verbose = progress_bar, epochs=epochs, callbacks=callbacks, validation_data=(Reshaped_validation_data, reshaped_val))
    return history
